...
export default function AdminCourses() {
  ...
  const simulateGenerate = (courseId: string, courseTitle: string) => {
    // Minimal, safe placeholder content — replace with your Bedrock generation result later
    dispatch(
      injectGeneratedModule({
        courseId,
        module: {
          id: `m-${Date.now()}`,
          title: `${courseTitle} — Module 1`,
          blocks: [
            { kind: 'paragraph', text: 'This module was generated from uploaded documents, links, and videos.' },
            { kind: 'paragraph', text: 'It introduces the topic and outlines key learning outcomes.' },
            {
              kind: 'media',
              media: { type: 'image', url: 'https://via.placeholder.com/800x420?text=Generated+Module', caption: 'Auto-generated visual' },
            },
          ],
          quiz: [
            {
    ...
    <Box>
            ...
            {courses.map((c) => (
              <TableRow key={c.id} hover>
                ...
                <TableCell>
                  <Chip
                    size="small"
                    color={c.modules?.length ? 'secondary' : 'default'}
                    label={`${c.modules?.length ?? 0}`}
                  />
                </TableCell>
                <TableCell>
                  <Chip
                    size="small"
                    color={c.finalExam ? 'secondary' : 'default'}
                    label={c.finalExam ? 'Yes' : 'No'}
                  />
                </TableCell>
      ...
      <Dialog open={open} onClose={() => setOpen(false)}>
        ...
        <DialogContent sx={{ pt: 2, display: 'grid', gap: 2, minWidth: 420 }}>
          ...
          />
          <TextField
            label="AWS Services (comma‑separated)"
            value={form.services}
            onChange={(e) => setForm({ ...form, services: e.target.value })}
            placeholder="Lambda, API Gateway, DynamoDB"
          />
        </DialogContent>
        <DialogActions>
